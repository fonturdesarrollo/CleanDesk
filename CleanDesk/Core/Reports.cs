    ...
    public class Reports : IReport
    {
        ...
        public List<RequestDetailModel> GetAllRequestsDetailQueue()
        {
            try
            {
                ...
                sqlConnection.Open();
                SqlCommand cmd = new($"SELECT * FROM View_LastRequestDetailsList WHERE RequestDetailStatusId <> 7 AND RequestDetailStatusId <> 8 AND RequestDetailStatusId <> 6  AND RequestDetailStatusId <> 4 AND RequestDetailStatusId <> 5 ORDER BY RequestDetailId", sqlConnection);
        ...
        public List<RequestDetailModel> GetResolveRequestsDetailQueue()
        {
            try
            {
                ...
                using SqlConnection sqlConnection = new(_configuration.GetConnectionString("connectionString"));
                sqlConnection.Open();
                SqlCommand cmd = new($"SELECT * FROM View_LastRequestDetailsList WHERE RequestDetailStatusId = 4 ORDER BY RequestDetailId", sqlConnection);
                SqlDataReader dr = cmd.ExecuteReader();
        ...
        public List<RequestDetailModel> GetUnresolveRequestsDetailQueue()
        {
            try
            {
                ...
                using SqlConnection sqlConnection = new(_configuration.GetConnectionString("connectionString"));
                sqlConnection.Open();
                SqlCommand cmd = new($"SELECT * FROM View_LastRequestDetailsList WHERE RequestDetailStatusId = 5 ORDER BY RequestDetailId", sqlConnection);
                SqlDataReader dr = cmd.ExecuteReader();
